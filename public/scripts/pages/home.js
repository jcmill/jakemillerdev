...
var scaleComponentCallback = function scaleComponentCallback(entries, observer) {
...
var scaleComponentObserver = new IntersectionObserver(scaleComponentCallback, scaleOptions);
portfolioComponent.forEach(function (target) {
  scaleComponentObserver.observe(target);
});
var portfolioPhrases = ["Like any good story, some chapters flow effortlessly… and others involve a lot of coffee and debugging at 2 AM.", "Every great build starts with a simple idea… and a few ‘why isn’t this working?", "Every pixel, every challenge, every late-night aha! moment.", "here’s a glimpse into the journey.", "Some projects feel like a walk in the park. Others…", "Like scaling a mountain with a laptop in one hand and coffee in the other.", "Some chapters involve battling the Kraken of cross-browser compatibility."];
var targetElements = document.querySelectorAll(".c-portfolio__component:nth-of-type(4n + 3)");
var style = document.createElement("style");
targetElements.forEach(function (el, i) {
  var uniqueClass = "portfolio-phrase-".concat(i);
  el.classList.add(uniqueClass);
  style.textContent += "\n    .".concat(uniqueClass, "::before {\n      content: \"").concat(portfolioPhrases[i], "\";\n    }\n  ");
});
...
function scaleImage(target, index) {
  ...
  var visiblePercentage = Math.min(Math.max((windowHeight - rect.top) / windowHeight, 0), 1);
