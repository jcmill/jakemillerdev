...
export default function ColorCurve({ piece }) {
  useLogoScroll();
  useIconObserver();
  ...
  return (
    <>
      <Header />
      <NavHamburger />
      <section className={`c-project ${portfolio[0].heroImageClass}`}>
        <div className="c-project__image">
          <img
            className="c-project__main-image"
            src={portfolio[0].heroImage}
            alt=""
          />
        </div>
      </section>
      <section className="c-project__details o-grid js-bgc--lt">
        <div className="c-project__details--prime">
          <div className="c-project__details--title">
            <h6 className="c-project__details--brand">{portfolio[0].brand}</h6>
            <h3 className="c-project__details--header">{portfolio[0].title}</h3>
          </div>
          <div className="c-project__details--role">
            <p>
              <b>Roll</b>
            </p>
            <p>{portfolio[0].role}</p>
            <p>
              <b>{portfolio[0].countryTitle}</b>
            </p>
            <p>{portfolio[0].country}</p>
          </div>
          {portfolio[0].hasBtn && (
            <div className="c-project__details--buttons">
              <a href={portfolio[0].btnUrl} target="_blank" rel="noreferrer">
                <button className="btn-outline-lt">
                  {portfolio[0].btnText}
                </button>
              </a>
              {portfolio[0].hasBtn2 && (
                <a href={portfolio[0].btnUrl2} target="_blank" rel="noreferrer">
                  <button className="btn-outline-lt">
                    {portfolio[0].btnText2}
                  </button>
                </a>
              )}
            </div>
      ...
      />
      {(portfolio[0].hasAltImagesSecond || portfolio.hasAltVideoSecond) && (
        <ProjectMediaAlt
          piece={portfolio[0]}
          classType={"c-project__full-media"}
