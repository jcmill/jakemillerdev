...
export default function GotrimRedesign({ piece }) {
  useLogoScroll();
  useIconObserver();
  ...
  return (
    <>
      <Header />
      <NavHamburger />
      <section className={`c-project ${portfolio[2].heroImageClass}`}>
        <div className="c-project__image">
          <img
            className="c-project__main-image"
            src={portfolio[2].heroImage}
            alt=""
          />
        </div>
      </section>
      <section className="c-project__details o-grid js-bgc--lt">
        <div className="c-project__details--prime">
          <div className="c-project__details--title">
            <h6 className="c-project__details--brand">{portfolio[2].brand}</h6>
            <h3 className="c-project__details--header">{portfolio[2].title}</h3>
          </div>
          <div className="c-project__details--role">
            <p>
              <b>Roll</b>
            </p>
            <p>{portfolio[2].role}</p>
            <p>
              <b>{portfolio[2].countryTitle}</b>
            </p>
            <p>{portfolio[2].country}</p>
          </div>
          {portfolio[2].hasBtn && (
            <div className="c-project__details--buttons">
              <a href={portfolio[2].btnUrl} target="_blank" rel="noreferrer">
                <button className="btn-outline-lt">
                  {portfolio[2].btnText}
                </button>
              </a>
              {portfolio[2].hasBtn2 && (
                <a href={portfolio[2].btnUrl2} target="_blank" rel="noreferrer">
                  <button className="btn-outline-lt">
                    {portfolio[2].btnText2}
                  </button>
                </a>
              )}
            </div>
      ...
      />
      {(portfolio[2].hasAltImagesSecond || portfolio.hasAltVideoSecond) && (
        <ProjectMediaAlt
          piece={portfolio[2]}
          classType={"c-project__full-media"}
