...
export default function ProductDetailPage({ piece }) {
  useLogoScroll();
  useIconObserver();
  ...
  return (
    <>
      <Header />
      <NavHamburger />
      <section className={`c-project ${portfolio[5].heroImageClass}`}>
        <div className="c-project__image">
          <img
            className="c-project__main-image"
            src={portfolio[5].heroImage}
            alt=""
          />
        </div>
      </section>
      <section className="c-project__details o-grid js-bgc--lt">
        <div className="c-project__details--prime">
          <div className="c-project__details--title">
            <h6 className="c-project__details--brand">{portfolio[5].brand}</h6>
            <h3 className="c-project__details--header">{portfolio[5].title}</h3>
          </div>
          <div className="c-project__details--role">
            <p>
              <b>Roll</b>
            </p>
            <p>{portfolio[5].role}</p>
            <p>
              <b>{portfolio[5].countryTitle}</b>
            </p>
            <p>{portfolio[5].country}</p>
          </div>
          {portfolio[5].hasBtn && (
            <div className="c-project__details--buttons">
              <a href={portfolio[5].btnUrl} target="_blank" rel="noreferrer">
                <button className="btn-outline-lt">
                  {portfolio[5].btnText}
                </button>
              </a>
              {portfolio[5].hasBtn2 && (
                <a href={portfolio[5].btnUrl2} target="_blank" rel="noreferrer">
                  <button className="btn-outline-lt">
                    {portfolio[5].btnText2}
                  </button>
                </a>
              )}
            </div>
      ...
      />
      {(portfolio[5].hasAltImagesSecond || portfolio.hasAltVideoSecond) && (
        <ProjectMediaAlt
          piece={portfolio[5]}
          classType={"c-project__full-media"}
